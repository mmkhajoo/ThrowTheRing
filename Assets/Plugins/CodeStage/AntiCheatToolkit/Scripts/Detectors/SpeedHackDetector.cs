...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class SpeedHackDetector : ActDetectorBase
	{
		...
		private const long TicksPerSecond = System.TimeSpan.TicksPerMillisecond * 1000;
		...
		public static void StartDetection(System.Action callback, float interval)
		{
		...
		public static void StartDetection(System.Action callback, float interval, byte maxFalsePositives)
		{
		...
		public static void StartDetection(System.Action callback, float interval, byte maxFalsePositives, int coolDown)
		{
		...
		private void Update()
		{
			...
			var timeCheated = Math.Abs((ticksFromStart) / (float)TicksPerSecond - (vulnerableTime - vulnerableTimeOnStart)) > ThresholdFloat;
			...
			if (ticksCheated || timeCheated)
			{
				...
				if (currentFalsePositives > maxFalsePositives)
				{
					...
					Debug.LogWarning(ACTkConstants.LogPrefix + "SpeedHackDetector: final detection!", this);
#endif
					OnCheatingDetected();
				}
				else
				{
#if ACTK_DEBUG_ENABLED
					Debug.LogWarning(ACTkConstants.LogPrefix + "SpeedHackDetector: detection! Allowed false positives left: " + (maxFalsePositives - currentFalsePositives), this);
#endif
			...
			{
#if ACTK_DEBUG_ENABLED
				Debug.LogWarning(ACTkConstants.LogPrefix + "SpeedHackDetector: success shot! Shots till cool down: " + (coolDown - currentCooldownShots), this);
#endif
				currentCooldownShots++;
				if (currentCooldownShots >= coolDown)
				{
#if ACTK_DEBUG_ENABLED
					Debug.LogWarning(ACTkConstants.LogPrefix + "SpeedHackDetector: cool down!", this);
#endif
