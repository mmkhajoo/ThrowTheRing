...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class WallHackDetector : ActDetectorBase
	{
		...
		public static void StartDetection(System.Action callback, Vector3 spawnPosition)
		{
		...
		public static void StartDetection(System.Action callback, Vector3 spawnPosition, byte maxFalsePositives)
		{
		...
		private void StartDetectionInternal(System.Action callback, Vector3 servicePosition, byte falsePositivesInRow)
		{
			if (isRunning)
			{
				...
				if (checkWireframe && wfCamera == null)
				{
					...
					else
					{
						...
						else
						{
							...
							{
								shaderTexture = new Texture2D(ShaderTextureSize, ShaderTextureSize, TextureFormat.RGB24, false);
							...
							{
								renderTexture = new RenderTexture(RenderTextureSize, RenderTextureSize, 24, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default);
							...
							if (targetTexture == null)
							{
								targetTexture = new Texture2D(RenderTextureSize, RenderTextureSize, TextureFormat.RGB24, false);
...
#if WALLHACK_DEBUG
				Debug.Log(FINAL_LogPrefix + "wireframe wallhack detected. Details below:\n" +
